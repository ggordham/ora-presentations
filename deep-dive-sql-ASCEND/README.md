...
```bash
podman exec ascenddb sh -c "curl -L https://github.com/ggordham/ora-presentations/tarball/main | tar xz --strip=1"

```

*Note ascenddb in the command is the container name, use your container name.  You can find your container name with the ```podman ps``` command.*
...
```bash
podman exec -it ascenddb sh -c "chmod +x splus.sh; /home/oracle/splus.sh ddsa"
