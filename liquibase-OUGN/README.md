...
```bash
curl -L https://github.com/ggordham/ora-presentations/tarball/main | tar xz --strip=1 "ggordham-ora-presentations-???????/liquibase-OUGN"
...
Download the hr-gold-exp.zip file, this contains the HR schema used for the demonstration.

```bash
cd liquibase-OUGN/setup
/usr/bin/curl -O https://raw.githubusercontent.com/ggordham/ora-presentations/main/ora-samp-schema/hr-gold-exp.zip
unzip -q hr-gold-exp.zip
```
...
_Note 2: if you are useing a true DEMO environment that can be destroyed, I've created a setup.sh script that will setup the PDBs and import the dump file from the zip file._
...
```bash
git diff --name-only | grep liquibase-OUGN
