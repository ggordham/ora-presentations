...
Scripts for Oracle Adaptive SQL Optimization - Why do things go so bad? TOUG presentation
...
```bash
podman exec TCOUG sh -c "curl -L https://github.com/ggordham/ora-presentations/tarball/main | tar xz --strip=1"
```

_Note TCOUG in the command is the container name, use your container name. You can find your container name with the podman ps command._
...
```bash
podman exec -it TCOUG sh -c "chmod +x splus.sh; /home/oracle/splus.sh adst"
```
...
## Optional ACS bucket demo

This demonstration will show the multiple buckets that Adaptive Cursor Sharing (ACS) uses to help determin if a bind variable value change (bind variable equality) would require a plan change.

Load the demo data, this will create a table with four buckets of records.  One set with two records, one with 500, one with 10,000, and finally one with 2,000,000.

```sql
@@acs_ddl.sql 500 10000 2000000
```

Run the demonstration:

```sql
@@acs_buckets_multi.sql 1 500 10000 2000000
```

_Note: this will run the queries multiple times and in sequential order. You can change the order of the bind variables if you would like to see how the Optimizer detects changes in different ways._

There is also a second script `acs_buckets.sql` that only runs each query once, where you may not see plan changes happen.

Optionally you can run a trace to see the bind equivalence actions happen:
...
To drop the sample schema in order to reload them or clean up run the following:
```SQL
@drop_sch.sql 
```
