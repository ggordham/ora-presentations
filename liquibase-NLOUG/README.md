...
NLOUG Database Cloud & Developer Day in Utrecht - November 13th, 2025
...
```bash
curl -L https://github.com/ggordham/ora-presentations/tarball/main | tar xz --strip=1 "ggordham-ora-presentations-???????/liquibase-NLOUG"
...
```bash
git diff --name-only | grep liquibase-NLOUG
