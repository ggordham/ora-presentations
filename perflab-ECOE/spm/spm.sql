...
prompt
prompt  We will lookup the plan hash value for the plan we want
prompt
set echo on
SELECT *
...
set echo off
prompt Look for the plan hash value above, and cut and paste it here.
accept myphv char prompt 'Enter the plan hash value of the bad NESTED LOOP plan (above): '

prompt
prompt We will now run the SQL Plan Management job to create a baseline
prompt  from the plan in SQL cache and accept that baseline.
prompt
