...
Scripts for Oracle AI database 26ai New SQL Performance Features presentation.
NLOUG Database Cloud & Developer Day in Utrecht - November 13th, 2025 - Oracle 26ai
...
These scripts require the usage of Oracle AI database 26ai.  As of the time of this writing, this version is only availabe in Oracle cloud, and in the FREE edition that can be run on a container, Linux, or Windows.
...
These test will be run with 26ai FREE edition https://www.oracle.com/database/free/

### Prerequisets
You need a working podman image with Oracle database FREE pre installed.
These scripts only work on 26ai FREE version.
...
```bash
podman exec nloug sh -c "curl -L https://github.com/ggordham/ora-presentations/tarball/main | tar xz --strip=1"
```

*Note nloug in the command is the container name, use your container name.  You can find your container name with the ```podman ps``` command.*
...
```bash
podman exec -it nloug sh -c "chmod +x splus.sh; /home/oracle/splus.sh sql26n"
...
podman cp nloug:/home/oracle/26ai-sql-perf-NLOUG/monitor_output.html .
...
You will need a Base Database System with Oracle database 26ai.
...
```sh
curl -L https://github.com/ggordham/ora-presentations/tarball/main | tar xz --strip=1 "ggordham-ora-presentations-???????/26ai-sql-perf-NLOUG"
...
The following steps will show real-time SPM in action.  Be sure you are using 26ai Enterprise Edition.
...
The following steps will show real-time SPM in action, this time moving from a good plan to a bad plan.  Be sure you are using 26ai Enterprise Edition.
First lets clear out any previous tests, and enable real-time SPM.
...
sqlplus /nolog <<'EOF'
@mksample.sql Oradb$_Lab42 Oradb$_Lab42 Oradb$_Lab42 Oradb$_Lab42 Oradb$_Lab42 Oradb$_Lab42 Oradb$_Lab42 Oradb$_Lab42 users temp /home/oracle/26ai-sql-perf-NLOUG/samp/ freepdb1
